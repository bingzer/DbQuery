...
abstract class BaseResolver implements IBaseResolver {
    ...
    final IContentQueryConfig contentConfig = new ContentConfig();
    final Uri uri;
    final ContentResolver contentResolver;

    BaseResolver(Uri uri, Context context){
    ...
    @Override
    public IContentQueryConfig getContentConfig() {
        return contentConfig;
    }

    @Override
    public String getPrimaryKeyColumn(){
        if(contentConfig.getAppendTableNameForId()){
            String tableName = "";
            Matcher m = getUriPattern().matcher(uri.toString());
            if(m.find()){
                tableName = m.group(3);
            }
            return tableName + contentConfig.getIdNamingConvention();
        }
        return contentConfig.getIdNamingConvention();
    }
    ...
    public InsertInto insertInto(String... columns) {
        return new ContentInsertIntoImpl(new ContentInsertIntoImpl.ContentSet() {
            @Override
            public void onContentValuesSet(ContentInsertIntoImpl query, ContentValues contentValues) {
    ...
    public <E extends IEntity> Insert insert(IEntityList<E> entityList) {
        if(contentConfig.getDefaultAuthority() == null)
        ...
        try{
            ContentProviderResult[] results =
                    contentResolver.applyBatch(getContentConfig().getDefaultAuthority(), operationList);
    ...
    public <E extends IEntity> Update update(IEntityList<E> entityList) {
        if(getContentConfig().getDefaultAuthority() == null)
